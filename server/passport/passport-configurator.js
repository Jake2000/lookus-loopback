...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  if (authType === 'local') {
    ...
    provider.callbacks.authCallback = passport.authenticate(name, options.fn || _.defaults({
    ...
    if(!options.attachOnly) {
      self.app.post(authPath, provider.callbacks.authCallback);
    }
  } else if (link) {
    provider.callbacks.authCallback = passport.authorize(name, _.defaults({scope: scope, session: session}, options.authOptions));

    if(!options.attachOnly) {
      self.app.get(authPath, provider.callbacks.authCallback);
    }
  } else {
    provider.callbacks.authCallback = passport.authenticate(name, _.defaults({scope: scope, session: session}, options.authOptions));
   ...
   */
  if (link) {
    provider.callbacks.finishCallback = passport.authorize(name, _.defaults({
      session: session,
    ...
    if(!options.attachOnly) {
      self.app.get(callbackPath, provider.callbacks.finishCallback,
    ...
    var customCallback = provider.callbacks.finishCallback = options.customCallback || function (req, res, next) {
...
PassportConfigurator.prototype.getCallbacks = function(name, callbackName) {
